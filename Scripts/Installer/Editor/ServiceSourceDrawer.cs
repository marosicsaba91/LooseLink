...
namespace UnityServiceLocator.Editor
{
    static class ServiceSourceDrawer
    {
        ...
        {
            for (var i = 0; i < containingProvider.SourceCount; i++)
            {
            ...
            if (containingProvider.IsSingleSourceProvider) return;
        ...
        static bool IsSourceEnabled
        {
            get
            {
                if (!_source.Enabled) return false;
                if (_containingProvider == null) return true;
                if (_containingProvider.GetType() != typeof(LocalServiceInstaller)) return true;
                if (string.IsNullOrEmpty(((LocalServiceInstaller)_containingProvider).gameObject.scene.name)) return true;
                if (((LocalServiceInstaller)_containingProvider).isActiveAndEnabled) return true;
                return false;
            }
        }
    }
